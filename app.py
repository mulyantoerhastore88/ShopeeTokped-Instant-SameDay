...
DEBUG_MODE = st.sidebar.checkbox("Tampilkan info detil (Debug)", value=True)
...
def process_universal_data(uploaded_files, kamus_data):
    try:
        ...
        for mp_type, file_obj in uploaded_files:
            ...
            # Find SKU, Qty, Order columns
            sku_col = next((c for c in df_filtered.columns if any(x in c for x in ['sku', 'referensi'])), None)
            ...
            for _, row in df_filtered.iterrows():
                raw_sku = str(row[sku_col]) if sku_col in row else ''
                sku_clean = clean_sku(raw_sku)
                order_id = str(row[order_col]) if order_col in row else ''
                
                try:
                    qty = float(str(row[qty_col]).replace(',', '.')) if qty_col in row else 0
                except:
                    qty = 0
    ...
    if not results['detail'].empty:
        ...
        with tab3:
            if not results['raw_stats'].empty:
                st.dataframe(results['raw_stats'], use_container_width=True)
        ...
        with pd.ExcelWriter(buf, engine='xlsxwriter') as writer:
            ...
            if not results['summary'].empty:
                results['summary'].to_excel(writer, sheet_name='Picking List', index=False)
            if not results['raw_stats'].empty:
...
st.sidebar.markdown("---")
st.sidebar.caption(f"v4.0 • {datetime.now().strftime('%d/%m/%Y')}")
